...
public class ListSetTest extends TestCase {
        ...
        private static void printStats(TestResults ref, TestResults test, final SequenceGenerator seq,
                ...
                System.out.println((test.addRate / ref.addRate) + "x improvement in add throughput (" + ref.addRate
                                + " -> " + test.addRate + ")");
                ...
                System.out.println((test.removeRate / ref.removeRate) + "x improvement in remove throughput ("
                                + ref.removeRate + " -> " + test.removeRate + ")");
                System.out.println("=========================================================");
        }

        public void testCoarseGrainedLockingRandomLarge() throws InterruptedException {
                final SequenceGenerator addSeq = new RandomSequenceGenerator(0, getNCores() * randNumsLength,
                                randNumRange);
                final SequenceGenerator containsSeq = new RandomSequenceGenerator(1, getNCores() * randNumsLength,
                                randNumRange);
                ...
                testCoarseGrainedLockingHelper(addSeq, containsSeq, removeSeq, expectedAdd, expectedContains,
                                expectedRemove, "Large");
        }

        public void testCoarseGrainedLockingRepeatingLarge() throws InterruptedException {
                final SequenceGenerator addSeq = new RepeatingSequenceGenerator(getNCores() * 6 * randNumsLength,
                                randNumsLength);
        ...
        public void testReadWriteLocksRandomLarge() throws InterruptedException {
                ...
                testReadWriteLocksHelper(addSeq, containsSeq, removeSeq, expectedAdd, expectedContains, expectedRemove,
                                "Large");
        }

        public void testReadWriteLocksRandomSmall() throws InterruptedException {
                final SequenceGenerator addSeq = new RandomSequenceGenerator(0, getNCores() * randNumsLength / 2,
                                randNumRange);
        ...
        public void testReadWriteLocksRepeatingSmall() throws InterruptedException {
                final SequenceGenerator addSeq = new RepeatingSequenceGenerator(getNCores() * 3 * randNumsLength,
                                randNumsLength);
        ...
        private void testCoarseGrainedLockingHelper(final SequenceGenerator addSeq, final SequenceGenerator containsSeq,
                ...
                final TestResultsPair results = runKernel(() -> new CoarseList(), "CoarseList", () -> new SyncList(),
        ...
        public void testReadWriteLocksHelper(final SequenceGenerator addSeq, final SequenceGenerator containsSeq,
                        final SequenceGenerator removeSeq, final double expectedAdd, final double expectedContains,
                        final double expectedRemove, final String datasetName) throws InterruptedException {
                final TestResultsPair results = runKernel(() -> new RWCoarseList(), "RWCoarseList",
                                () -> new SyncList(), "SyncList", addSeq, containsSeq, removeSeq);
                ...
                final String addmsg = String.format("Expected add throughput "
                                + "improvement to be at least %fx with read-write locks, but " + "found %fx",
                                expectedAdd, addImprovement);
                assertTrue(addmsg, addImprovement >= expectedAdd);
        ...
        private static TestResultsPair runKernel(final ListFactory factoryA, final String lblA,
                        final ListFactory factoryB, final String lblB, final SequenceGenerator addSeq,
                        final SequenceGenerator containsSeq, final SequenceGenerator removeSeq)
                ...
                for (int r = 0; r < 5; r++) {
                        ...
                        TestResults newResults = mainKernel(numThreads, factoryA.construct(), lblA, addSeq, containsSeq,
                                        removeSeq);
        ...
        private static TestResults mainKernel(final int numThreads, final ListSet list, final String lbl,
                ...
                while (curr != null) {
                        assertTrue("List was not sorted, index " + (listLengthAfterAdds - 1) + " is "
                                        + prev.object.intValue() + " and index " + listLengthAfterAdds + " is "
                                        + curr.object.intValue(), curr.object.intValue() > prev.object.intValue());
                ...
                for (int t = 0; t < numThreads; t++) {
                        containsRunnables[t] = new ContainsTestThread(containsSeq,
                                        containsSeq.sequenceLength() / numThreads, list);
                }
                ...
                for (int t = 0; t < numThreads; t++) {
                        removeRunnables[t] = new RemoveTestThread(removeSeq, removeSeq.sequenceLength() / numThreads,
                                        list);
                }
                ...
                return new TestResults(lbl, addRate, containsRate, removeRate, listLengthAfterAdds,
                                totalContainsSuccesses, totalContainsFailures, listLengthAfterRemoves,
        ...
        private static class TestResults {
                ...
                public TestResults(final String lbl, final double addRate, final double containsRate,
                                final double removeRate, final int listLengthAfterAdds,
                                final int totalContainsSuccesses, final int totalContainsFailures,
                                final int listLengthAfterRemoves, final int totalRemovesSuccesses,
